...
import { adminAPI, sessionsAPI, spacesAPI, servicesAPI, couponsAPI } from '../utils/api';
...
export default function AdminDashboard() {
  ...
  const [couponFilter,   setCouponFilter]   = useState('all'); // all | active | used
  const [newCoupon, setNewCoupon] = useState({
    code:        '',       // فارغ = يتولد تلقائياً
    discount:    20,       // نسبة الخصم %
    days:        30,       // صلاحية بالأيام
    targetType:  'global', // global | user
    targetUser:  '',       // اسم أو موبايل العميل
  });
  const [couponUsers,     setCouponUsers]     = useState([]); // نتايج البحث عن عميل
  ...
  // ✅ جيب كل الكوبونات (نظام + يدوية) — محتاج endpoint في الـ backend
  ...
  async function chargeWallet(u) {
    ...
    try {
      ...
      setAmount(u.id, 'wallet', ''); loadUsers();
  ...
  async function addPoints(u) {
    ...
    try {
      ...
      setAmount(u.id, 'points', ''); loadUsers();
    } catch (err) { toast.error(err.response?.data?.error || 'خطأ'); }
  }
  ...
  async function createCoupon() {
    ...
    try {
      const payload = {
        code:        newCoupon.code.trim().toUpperCase() || null, // null = يتولد تلقائياً
      ...
      const { data } = await couponsAPI.adminCreate(payload);
      toast.success(`✅ تم إنشاء الكوبون: ${data.coupon.code}`);
      // reset form
      setNewCoupon({ code: '', discount: 20, days: 30, targetType: 'global', targetUser: '' });
  ...
  return (
    <div style={{ minHeight: '100vh', maxWidth: 680, margin: '0 auto', padding: '0 0 40px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, background: 'var(--bg)', zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => navigate('/subscriptions')}
            style={{ background: 'transparent', border: '1px solid var(--accent)', color: 'var(--accent)', padding: '6px 12px', borderRadius: 8, fontSize: 12, cursor: 'pointer' }}>📋 الاشتراكات</button>
      ...
      <div style={{ display: 'flex', gap: 4, padding: '12px 16px', borderBottom: '1px solid var(--border)', overflowX: 'auto' }}>
        {[['overview','نظرة عامة'], ['users','العملاء'], ['prices','الأسعار'], ['coupons','🎫 الكوبونات']].map(([k, label]) => (
        ...
        <button onClick={() => navigate('/scanner')}
          style={{ padding: '7px 16px', borderRadius: 20, border: '1px solid var(--border)', fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap', cursor: 'pointer', background: 'transparent', color: 'var(--muted)' }}>📡 Scanner</button>
      </div>
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 16, overflowX: 'auto' }}>
              {[['cowork','🖥️ منطقة العمل'], ['meeting','🤝 الاجتماعات'], ['lessons','📚 الدروس'], ['services','☕ الخدمات']].map(([k, label]) => (
                <button key={k} onClick={() => setPriceTab(k)}
                  style={{ padding: '7px 14px', borderRadius: 20, border: '1px solid', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap', cursor: 'pointer', borderColor: priceTab === k ? 'var(--accent)' : 'var(--border)', background: priceTab === k ? 'var(--accent)' : 'transparent', color: priceTab === k ? '#000' : 'var(--muted)' }}>{label}</button>
              ))}
            </div>
            ...
            {priceTab === 'cowork' && (
              <div className="card">
                ...
                <div style={{ padding: '8px 12px', background: 'rgba(0,212,170,0.06)', borderRadius: 8, fontSize: 12, color: 'var(--muted)', margin: '14px 0 12px' }}>
                  ...
                  90 دقيقة = <strong style={{ color: 'var(--accent)' }}>{spaces.cowork.first_hour * 2} ج</strong> &nbsp;|&nbsp;
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 14 }}>
                  {[['first_hour','سعر أول ساعة'], ['extra_hour','كل ساعة إضافية']].map(([key, label]) => (
                    <div key={key}>
                      ...
                      <NumberInput value={spaces.meeting[key]} onChange={val => setSpaces(prev => ({ ...prev, meeting: { ...prev.meeting, [key]: parseFloat(val) || 1 } }))} min={1} step={1} suffix="ج" />
                    </div>
                  ))}
                </div>
              ...
              <div className="card">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 14 }}>
                  {[['first_hour','سعر أول ساعة'], ['extra_hour','كل ساعة إضافية']].map(([key, label]) => (
                    <div key={key}>
                      <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 8 }}>{label} (ج)</div>
                      <NumberInput value={spaces.lessons[key]} onChange={val => setSpaces(prev => ({ ...prev, lessons: { ...prev.lessons, [key]: parseFloat(val) || 1 } }))} min={1} step={1} suffix="ج" />
                    </div>
                  ))}
                </div>
                <div style={{ padding: '8px 12px', background: 'rgba(0,212,170,0.06)', borderRadius: 8, fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>
                  ...
                  {services.map(s => (
                    <div key={s.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      {editingService?.id === s.id ? (
                        <>
                          <input className="input-field" style={{ flex: 1 }} value={editingService.name} onChange={e => setEditingService(prev => ({ ...prev, name: e.target.value }))} />
                          <input className="input-field" type="number" style={{ width: 80 }} value={editingService.price} onChange={e => setEditingService(prev => ({ ...prev, price: e.target.value }))} />
          ...
          <div className="fade-up">
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 6 }}>
                كود الكوبون <span style={{ opacity: 0.6 }}>(اتركه فارغاً للتوليد التلقائي)</span>
              </div>
              <input className="input-field" style={{ marginBottom: 14, textTransform: 'uppercase', letterSpacing: 1 }}
                placeholder="مثال: RAMADAN30 — أو اتركه فارغاً"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 14 }}>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8 }}>نسبة الخصم %</div>
                  <NumberInput
                    value={newCoupon.discount}
                    onChange={val => setNewCoupon(prev => ({ ...prev, discount: val }))}
                    min={1} step={1} suffix="%"
                  />
                </div>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8 }}>الصلاحية (أيام)</div>
                  <NumberInput
                    value={newCoupon.days}
                    onChange={val => setNewCoupon(prev => ({ ...prev, days: val }))}
                    min={1} step={1} suffix="يوم"
                  />
                </div>
              </div>

              {/* النوع: عام أم لعميل معين */}
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                {[['global','🌐 عام (لأي عميل)'], ['user','👤 لعميل معين']].map(([type, label]) => (
                  <button key={type}
                    onClick={() => { setNewCoupon(prev => ({ ...prev, targetType: type, targetUser: '' })); setSelectedCouponUser(null); setCouponUsers([]); }}
                    style={{ flex: 1, padding: '9px', borderRadius: 10, border: '1px solid', fontSize: 12, fontWeight: 600, cursor: 'pointer', borderColor: newCoupon.targetType === type ? 'var(--accent)' : 'var(--border)', background: newCoupon.targetType === type ? 'rgba(0,212,170,0.1)' : 'transparent', color: newCoupon.targetType === type ? 'var(--accent)' : 'var(--muted)' }}>
                    {label}
                  </button>
                ))}
              </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  {selectedCouponUser ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'rgba(0,212,170,0.08)', border: '1px solid rgba(0,212,170,0.3)', borderRadius: 10 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>{selectedCouponUser.name}</div>
                        <div style={{ fontSize: 12, color: 'var(--muted)' }}>{selectedCouponUser.phone}</div>
                      </div>
                      <button onClick={() => { setSelectedCouponUser(null); setNewCoupon(prev => ({ ...prev, targetUser: '' })); }}
                    ...
                    <div style={{ position: 'relative' }}>
                      <input className="input-field"
                        ...
                        onChange={e => { setNewCoupon(prev => ({ ...prev, targetUser: e.target.value })); searchCouponUsers(e.target.value); }} />
                        ...
                        <div style={{ position: 'absolute', top: '100%', right: 0, left: 0, zIndex: 20, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, marginTop: 4, overflow: 'hidden' }}>
                          {couponUsers.map(u => (
                            <div key={u.id} onClick={() => { setSelectedCouponUser(u); setNewCoupon(prev => ({ ...prev, targetUser: u.name })); setCouponUsers([]); }}
                              ...
                              <div style={{ fontSize: 11, color: 'var(--muted)' }}>{u.phone}</div>
                            </div>
                          ))}
                        </div>
                      )}
              ...
              <div style={{ padding: '8px 12px', background: 'rgba(0,212,170,0.06)', borderRadius: 8, fontSize: 12, color: 'var(--muted)', marginBottom: 14 }}>
                خصم <strong style={{ color: 'var(--accent)' }}>{newCoupon.discount}%</strong>
                &nbsp;·&nbsp; صالح <strong style={{ color: 'var(--accent)' }}>{newCoupon.days} يوم</strong>
                &nbsp;·&nbsp; {newCoupon.targetType === 'global' ? 'لأي عميل' : selectedCouponUser ? `لـ ${selectedCouponUser.name}` : 'حدد عميلاً'}
                {newCoupon.code && <>&nbsp;·&nbsp; كود: <strong style={{ color: 'var(--accent)', fontFamily: 'var(--mono)' }}>{newCoupon.code}</strong></>}
              </div>

              <button className="btn btn-primary" style={{ width: '100%' }}
                disabled={couponLoading || (newCoupon.targetType === 'user' && !selectedCouponUser)}
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {[['all','الكل'], ['active','فعال'], ['used','مستخدم']].map(([f, label]) => (
                  <button key={f} onClick={() => setCouponFilter(f)}
                    style={{ padding: '4px 10px', borderRadius: 20, border: '1px solid', fontSize: 11, fontWeight: 600, cursor: 'pointer', borderColor: couponFilter === f ? 'var(--accent)' : 'var(--border)', background: couponFilter === f ? 'var(--accent)' : 'transparent', color: couponFilter === f ? '#000' : 'var(--muted)' }}>{label}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filteredCoupons.length === 0 && (
                <div style={{ textAlign: 'center', color: 'var(--muted)', padding: 24, fontSize: 13 }}>لا توجد كوبونات</div>
              )}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                          ...
                          &nbsp;·&nbsp; ينتهي {format(new Date(c.expires_at), 'dd MMM yyyy', { locale: ar })}
                        </div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
