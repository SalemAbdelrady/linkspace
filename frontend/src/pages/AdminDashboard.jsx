...
function UserModal({ u, isInSession, amounts, getAmount, setAmount, chargeWallet, addPoints, onClose }) {
  if (!u) return null;
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      onClick={onClose}>
      <div style={{ background: 'var(--surface)', borderRadius: 20, padding: 24, maxWidth: 400, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: 'transparent', border: 'none', color: 'var(--muted)', fontSize: 24, cursor: 'pointer', lineHeight: 1 }}>✕</button>
        </div>

        {/* الرصيد والنقاط */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
          <div style={{ background: 'rgba(0,212,170,0.08)', borderRadius: 12, padding: '12px 16px', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>الرصيد</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--accent)' }}>{parseFloat(u.balance).toFixed(2)} ج</div>
          </div>
          <div style={{ background: 'rgba(255,165,2,0.08)', borderRadius: 12, padding: '12px 16px', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>النقاط</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--warning)' }}>{u.points}</div>
          </div>
        </div>
          ...
          <div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: '1px dashed var(--border)' }}>
            ...
            {services.map((s, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}>
                <span>{s.name} × {s.qty}</span><span>{(s.price * s.qty).toFixed(2)} ج</span>
              </div>
            ))}
          </div>
        )}
...
export default function AdminDashboard() {
  ...
  async function loadInvoices() {
    try {
      ...
      setInvoices(data.invoices); setInvoiceTotal(data.total);
                ...
                return (
                  ...
                  <div key={u.id} onClick={() => setSelectedUser(u)} className="card"
                    ...
                    <div style={{ display: 'flex', gap: 16, marginTop: 10, fontSize: 13 }}>
                      ...
                      {u.qr_code && <span style={{ fontFamily: 'var(--mono)', color: 'var(--muted)', fontSize: 11 }}>#{u.qr_code}</span>}
                    </div>
                  </div>
                );
              })}
        ...
        {tab === 'coupons' && (
          <div className="fade-up">
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <input className="input-field" style={{ marginBottom: 14, textTransform: 'uppercase', letterSpacing: 1 }} placeholder="مثال: RAMADAN30" value={newCoupon.code} onChange={e => setNewCoupon(prev => ({ ...prev, code: e.target.value.toUpperCase() }))} />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 14 }}>
        ...
        {tab === 'invoices' && (
          <div className="fade-up">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8, marginBottom: 14 }}>
              <input className="input-field" placeholder="بحث باسم العميل أو موبايله..." value={invoiceSearch} onChange={e => { setInvoiceSearch(e.target.value); setInvoicePage(1); }} />
              <input type="date" className="input-field" style={{ width: 150 }} value={invoiceDate} onChange={e => { setInvoiceDate(e.target.value); setInvoicePage(1); }} />
            </div>
            <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 10 }}>
              {invoiceTotal} فاتورة {invoiceDate && `— ${new Date(invoiceDate).toLocaleDateString('ar-EG',{month:'long',day:'numeric'})}`}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
              {invoices.length === 0 && <div style={{ textAlign: 'center', color: 'var(--muted)', padding: 32, fontSize: 13 }}>لا توجد فواتير</div>}
              {invoices.map(inv => {
            ...
            {totalInvoicePages > 1 && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: 8 }}>
                <button onClick={() => setInvoicePage(p => Math.max(1,p-1))} disabled={invoicePage===1} style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: invoicePage===1?'var(--muted)':'var(--text)', cursor: invoicePage===1?'default':'pointer' }}>السابق</button>
                <span style={{ padding: '6px 12px', fontSize: 13, color: 'var(--muted)' }}>{invoicePage} / {totalInvoicePages}</span>
                <button onClick={() => setInvoicePage(p => Math.min(totalInvoicePages,p+1))} disabled={invoicePage===totalInvoicePages} style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: invoicePage===totalInvoicePages?'var(--muted)':'var(--text)', cursor: invoicePage===totalInvoicePages?'default':'pointer' }}>التالي</button>
              </div>
            )}
          </div>
        )}
