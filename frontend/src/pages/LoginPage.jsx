        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3 }}>
              Link Space — آخر تحديث: مايو 2026
            </div>
          </div>
          <button
            onClick={onClose}
...
export default function LoginPage() {
  ...
  function validate() {
    if (mode === "register") {
      ...
      if (form.password.length < 6) return "كلمة السر 6 أحرف على الأقل";
      if (form.password !== form.confirmPassword)
        return "كلمتا السر غير متطابقتين";
      if (!form.agreeTerms) return "يجب الموافقة على سياسة الخصوصية للمتابعة";
    }
  ...
  const submit = async (e) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // بيانات غلط أو محظور — متحاولش تاني
        if (
          status === 401 ||
          status === 403 ||
          status === 404 ||
        ...
        // cold start أو network error — استنى وحاول تاني
        if (attempt < maxRetries) {
          await new Promise((r) => setTimeout(r, 1500));
          // مفيش toast هنا — المستخدم مش هيحس بحاجة
        }
      }
    }
          ...
          <form onSubmit={submit}>
            ...
            <div style={fieldWrap}>
              ...
              {mode === "register" &&
                ...
                form.password.length < 6 && (
                  <div style={{ fontSize: 10, color: "#ff4757", marginTop: 4 }}>
                    ⚠️ 6 أحرف على الأقل
                  </div>
                )}
            </div>
              ...
              <div style={fieldWrap}>
                ...
                {form.confirmPassword &&
                  form.password !== form.confirmPassword && (
                    <div
                      style={{ fontSize: 10, color: "#ff4757", marginTop: 4 }}
                    >
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  loading || (mode === "register" && !form.agreeTerms)
                    ? "rgba(0,212,170,0.4)"
                    : "var(--accent)",
                color: "#000",
                fontSize: 14,
