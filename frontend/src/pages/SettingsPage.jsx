...
export default function SettingsPage() {
  ...
  async function saveInfo() {
    ...
    if (!lastName.trim()) return toast.error("اسم العائلة مطلوب");
    const fullName = `${firstName.trim()} ${lastName.trim()}`;
    if (!isValidName(fullName))
      return toast.error("الاسم يجب أن يحتوي على حروف فقط");
  ...
  async function changePassword() {
    if (!currentPass || !newPass || !confirmPass)
      return toast.error("أدخل كل الحقول");
    if (newPass !== confirmPass)
      return toast.error("كلمة السر الجديدة غير متطابقة");
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <div
            style={{ fontSize: 17, fontWeight: 800, color: "var(--accent)" }}
          >
            ⚙️ إعدادات الحساب
          </div>
