...
export default function SettingsPage() {
  ...
  async function saveInfo() {
    ...
    if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return toast.error('البريد الإلكتروني غير صحيح');
  ...
  return (
    <div style={{ minHeight: '100vh', maxWidth: 480, margin: '0 auto', padding: '0 0 60px' }}>
      ...
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, background: 'var(--bg)', zIndex: 10 }}>
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* Avatar */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px', background: 'rgba(0,212,170,0.06)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: 16 }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'linear-gradient(135deg, var(--accent), var(--accent2))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 20, color: '#fff', flexShrink: 0 }}>
            {(user?.name || 'U').split(' ').slice(0, 2).map(w => w[0]).join('')}
          </div>
          <div>
        ...
        {/* ── بيانات الحساب ── */}
        <div className="card">
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 16, color: 'var(--accent)' }}>👤 البيانات الشخصية</div>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 12, color: 'var(--muted)', display: 'block', marginBottom: 6 }}>
              البريد الإلكتروني
              {!user?.email && <span style={{ color: 'var(--warning)', marginRight: 6, fontSize: 11 }}>⚠️ مطلوب لاستعادة كلمة السر</span>}
            </label>
            <input
          ...
          {!user?.email && (
            <div style={{ padding: '10px 14px', background: 'rgba(255,165,2,0.08)', border: '1px solid rgba(255,165,2,0.3)', borderRadius: 10, fontSize: 12, color: 'var(--warning)', marginBottom: 16 }}>
        ...
        {/* ── تغيير كلمة السر ── */}
        <div className="card">
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 16, color: 'var(--accent)' }}>🔐 تغيير كلمة السر</div>
