...
function SubscribeModal({ plans, onClose, onSuccess }) {
  ...
  async function confirmSubscription() {
    ...
    try {
      ...
      toast.success(
        `✅ تم تسجيل اشتراك ${selectedUser.name} في ${selectedPlan.name}`,
      );
      onSuccess(data);
      onClose();
    } catch (err) {
            ...
            {searchResults.map((u) => (
              ...
              >
                ...
                <div style={{ fontSize: 12, color: "var(--muted)" }}>
                  {u.phone} — رصيد: {parseFloat(u.balance).toFixed(2)} ج
                </div>
              </div>
            ))}
                ...
                >
                  <div>
                    ...
                    {plan.discount_rooms > 0 && (
                      ...
                      >
                        🎁 خصم {plan.discount_rooms}% على الغرف
                      </div>
                    )}
                  </div>
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    borderColor:
                      payMethod === m ? "var(--accent)" : "var(--border)",
                    background:
                      payMethod === m ? "var(--accent)" : "transparent",
                    color: payMethod === m ? "#000" : "var(--muted)",
                  }}
                >
                  {label}
                </button>
                ...
                style={{
                  ...
                  background:
                    parseFloat(selectedUser.balance) >= selectedPlan.price
                      ...
                      : "rgba(255,71,87,0.08)",
                  border: `1px solid ${parseFloat(selectedUser.balance) >= selectedPlan.price ? "rgba(46,213,115,0.4)" : "rgba(255,71,87,0.3)"}`,
                  borderRadius: 8,
                  fontSize: 12,
                  marginBottom: 14,
                  color:
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          className="btn btn-primary"
          style={{ marginRight: "auto", padding: "7px 16px", fontSize: 13 }}
                ...
                >
                  ...
                  >
                    ...
                    <input
                      ...
                      type="number"
                      style={{ width: 80 }}
                      value={newPlan.discount_rooms}
                      onChange={(e) =>
                        setNewPlan((p) => ({
                          ...p,
                          discount_rooms: e.target.value,
                        }))
                      }
                    />
                    <span style={{ fontSize: 13, color: "var(--muted)" }}>
                      ...
                      >
                        ...
                        >
                          ...
                          <input
                            ...
                            type="number"
                            style={{ width: 80 }}
                            value={editingPlan.discount_rooms}
                            onChange={(e) =>
                              setEditingPlan((p) => ({
                                ...p,
                                discount_rooms: e.target.value,
            ...
            {activeSubs.map((sub) => {
              ...
              return (
                <div
                  key={sub.id}
                  className="card"
                  style={{ marginBottom: 10, opacity: isExpired ? 0.6 : 1 }}
                >
                  <div
                    style={{
                      display: "flex",
                  ...
                  {/* أضف عرض سبب الإلغاء في بطاقة الاشتراك */}                  
                  {sub.cancel_reason && (
                    <div style={{ marginTop: 8, padding: "8px 12px",
                      background: "rgba(255,71,87,0.06)", border: "1px solid rgba(255,71,87,0.2)",
                      borderRadius: 8, fontSize: 12, color: "#ff4757" }}>
                      💬 سبب الإلغاء: {sub.cancel_reason}
                    </div>
                  )}
                  <div
                    style={{
                  ...
                  >
                    ...
                    {!isExpired && (
                      <button
                        onClick={() => cancelSub(sub.id, sub.client_name)}
                        style={{
                          background: "transparent",
                          border: "1px solid rgba(255,71,87,0.3)",
                          color: "#ff4757",
      ...
      {cancelModal && (
        ...
        >
          ...
          >
            ...
            >
              سبب الإلغاء{" "}
              <span style={{ opacity: 0.5 }}>(اختياري — سيظهر للعميل)</span>
            </div>
            <textarea
              className="input-field"
              placeholder="مثال: طلب العميل إيقاف الاشتراك..."
              value={cancelReason}
