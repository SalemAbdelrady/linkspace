...
// GET /api/coupons/validate — للتحقق السريع من الكوبون [staff/admin]
router.get('/validate', auth, requireRole('staff', 'admin'), async (req, res) => {
  ...
  try {
    const { rows } = await db.query(`
      ...
      WHERE c.code = $1
        AND (c.is_used = false OR (c.max_uses > 1 AND c.used_count < c.max_uses))
...
// GET /api/coupons/admin/:id/details — تفاصيل كوبون مع مستخدميه
router.get('/admin/:id/details', auth, requireRole('admin'), async (req, res) => {
  try {
    ...
    if (!coupon[0]) return res.status(404).json({ error: 'كوبون غير موجود' });

    const { rows: usages } = await db.query(`
      SELECT 
        i.id, i.invoice_number, i.client_name, i.client_phone,
        i.total, i.discount_amount, i.created_at,
        u.id AS user_id, u.avatar_url
      FROM invoices i
      LEFT JOIN users u ON u.id = i.user_id
      WHERE i.coupon_code = $1
      ORDER BY i.created_at DESC
    `, [coupon[0].code]);

    res.json({ coupon: coupon[0], usages });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'خطأ في الخادم' });
  }
...
router.post('/admin/create', auth, requireRole('admin', 'staff'), async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(`
      INSERT INTO coupons (user_id, code, discount_pct, expires_at, max_uses)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `, [user_id || null, finalCode, parseInt(discount), expiresAt, parseInt(max_uses)]);
